...
$rhino7Zip = Join-Path $dist "WoodSheetLayout-2.1.6-rhino7.zip"
Compress-Archive -Path (@((Join-Path $dist "net48")) + $common) -DestinationPath $rhino7Zip -Force

$rhino8Zip = Join-Path $dist "WoodSheetLayout-2.1.6-rhino8.zip"
Compress-Archive -Path (@((Join-Path $dist "net8.0")) + $common) -DestinationPath $rhino8Zip -Force

$combinedZip = Join-Path $dist "WoodSheetLayout-2.1.6-rhino7-rhino8.zip"
...
Write-Host "WoodSheetLayout 2.1.6 编译完成。" -ForegroundColor Green
