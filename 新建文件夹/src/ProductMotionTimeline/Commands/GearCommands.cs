...
namespace ProductMotionTimeline.Commands
{
  internal static class GearCommandRunner
  {
    internal static Result Run(RhinoDoc doc, GearPartType type)
    {
      ...
      RhinoApp.WriteLine(
        "ProductMotion：已生成 {0}，并自动建立时间轴轨道。{1}",
        name,
    ...
    internal static Result ChooseAndRun(RhinoDoc doc)
    {
      ...
      var spur = getter.AddOption("Spur");
      var internalGear = getter.AddOption("Internal");
      var helical = getter.AddOption("Helical");
      var bevel = getter.AddOption("Bevel");
