...
- 机械约束栏移除“外啮合齿轮、内啮合齿轮、皮带传动、同轴复合齿轮”四个重复按钮。
- 保留“一主多从/串联”，运行后统一选择 `Auto`、外啮合、内啮合、皮带、同轴刚性/复合齿轮等传动类型。
- 保留“编辑选中、检查全部、删除选中”和传动关系图定位功能。
- 旧单类型命令只在底层保留兼容，不再作为面板入口或推荐操作方式。
- 0.4.12 的黑色画布铺满、双滚动、关键帧与齿轮撤销等功能全部保留。
...
- 当时新增专用同轴入口；0.4.13 起面板统一使用“一主多从/串联”，并在类型中选择“同轴刚性或复合齿轮”。
...
1. 先在时间轴或Rhino视口选中主动件，物体与轨道会双向同步。
2. 点击“一主多从/串联”或运行 `PMTBindMultiple`。
3. 选择 `Auto`、外啮合、内啮合、皮带或同轴刚性/复合齿轮。
4. 在视口选择一个或多个从动件，按回车确认。
5. 只给最上游主动件设置关键帧；普通旋转可使用Gumball，需要整圈或多圈时输入 `360°`、`720°`。

统一绑定会自动寻找零件轴孔。若零件有多个相似圆孔，请用“设轴心”手动覆盖，或在选中轨道后重新运行 `PMTAutoPivot`。
...
2. 在时间轴中选中先获得动力的下层大齿轮，点“一主多从/串联”，选择“同轴刚性或复合齿轮”。
