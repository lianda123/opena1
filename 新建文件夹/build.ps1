...
if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) {
  throw "未找到 dotnet。请安装 .NET 7 SDK、.NET Framework 4.8 Developer Pack 与 .NET 桌面开发。"
}
...
New-Item -ItemType Directory -Path (Join-Path $dist "net7.0") -Force | Out-Null
...
Copy-Item -LiteralPath $rhino8Plugin -Destination (Join-Path $dist "net7.0\ProductMotionTimeline.rhp")
...
foreach ($extension in @("pdb", "xml")) {
  ...
  if (Test-Path $file8) { Copy-Item -LiteralPath $file8 -Destination (Join-Path $dist "net7.0") }
}
...
$zipPath = Join-Path $dist "ProductMotionTimeline-0.1.0-release.zip"
$releaseContents = @(
  (Join-Path $dist "net48"),
  (Join-Path $dist "net7.0"),
...
Compress-Archive -Path $releaseContents -DestinationPath $zipPath -Force
