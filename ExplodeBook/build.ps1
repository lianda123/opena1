...
$solution = Join-Path $projectRoot "ExplodeBook.sln"
$project = Join-Path $projectRoot "src\ExplodeBook\ExplodeBook.csproj"
$dist = Join-Path $projectRoot "dist"
$net48Out = Join-Path $projectRoot "src\ExplodeBook\bin\$Configuration\net48"
$net8Out = Join-Path $projectRoot "src\ExplodeBook\bin\$Configuration\net8.0"
...
$rhino7Plugin = Join-Path $net48Out "ExplodeBook.rhp"
$rhino8Plugin = Join-Path $net8Out "ExplodeBook.rhp"
...
Copy-Item -LiteralPath $rhino7Plugin -Destination (Join-Path $dist "net48\ExplodeBook.rhp")
Copy-Item -LiteralPath $rhino8Plugin -Destination (Join-Path $dist "net8.0\ExplodeBook.rhp")
foreach ($extension in @("pdb", "xml")) {
  $file7 = Join-Path $net48Out "ExplodeBook.$extension"
  $file8 = Join-Path $net8Out "ExplodeBook.$extension"
