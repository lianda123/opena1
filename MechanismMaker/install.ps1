...
$version = "1.0.0"
$installRoot = Join-Path $env:LOCALAPPDATA "MechanismMaker\$version"
$targets = if ($RhinoVersion -eq "Both") { @("7", "8") } else { @($RhinoVersion) }

foreach ($major in $targets) {
  $framework = if ($major -eq "7") { "net48" } else { "net8.0" }
  $source = Join-Path $PSScriptRoot "$framework\MechanismMaker.rhp"
  if (-not (Test-Path $source)) {
    throw "安装包中缺少 $framework\MechanismMaker.rhp。请先完整解压 ZIP。"
  }

  $destination = Join-Path $installRoot "Rhino$major"
  New-Item -ItemType Directory -Path $destination -Force | Out-Null
  Copy-Item -LiteralPath $source -Destination (Join-Path $destination "MechanismMaker.rhp") -Force
  $pluginPath = Join-Path $destination "MechanismMaker.rhp"
  ...
  New-ItemProperty -Path $regPath -Name "Name" -Value "MechanismMaker" -PropertyType String -Force | Out-Null
...
Write-Host "安装完成，请完全重启 Rhino，然后输入 MechanismMaker。" -ForegroundColor Green
