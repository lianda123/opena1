...
foreach ($major in $targets) {
  ...
  $source = Join-Path $PSScriptRoot "$framework\WoodCheck.rhp"
  if (-not (Test-Path $source)) {
    throw "安装包中缺少 $framework\WoodCheck.rhp。请先完整解压 ZIP。"
  }

  $destination = Join-Path $installRoot "Rhino$major"
  New-Item -ItemType Directory -Path $destination -Force | Out-Null
  Copy-Item -LiteralPath $source -Destination (Join-Path $destination "WoodCheck.rhp") -Force
  $pluginPath = Join-Path $destination "WoodCheck.rhp"
  ...
  New-ItemProperty -Path $regPath -Name "Name" -Value "WoodCheck" -PropertyType String -Force | Out-Null
...
Write-Host "安装完成，请完全重启 Rhino，然后输入 WoodCheck。" -ForegroundColor Green
