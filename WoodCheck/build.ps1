...
$solution = Join-Path $projectRoot "WoodCheck.sln"
$project = Join-Path $projectRoot "src\WoodCheck\WoodCheck.csproj"
$dist = Join-Path $projectRoot "dist"
$net48Out = Join-Path $projectRoot "src\WoodCheck\bin\$Configuration\net48"
$net8Out = Join-Path $projectRoot "src\WoodCheck\bin\$Configuration\net8.0"
...
$rhino7Plugin = Join-Path $net48Out "WoodCheck.rhp"
$rhino8Plugin = Join-Path $net8Out "WoodCheck.rhp"
...
Copy-Item -LiteralPath $rhino7Plugin -Destination (Join-Path $dist "net48\WoodCheck.rhp")
Copy-Item -LiteralPath $rhino8Plugin -Destination (Join-Path $dist "net8.0\WoodCheck.rhp")

foreach ($extension in @("pdb", "xml")) {
  $file7 = Join-Path $net48Out "WoodCheck.$extension"
  $file8 = Join-Path $net8Out "WoodCheck.$extension"
...
Write-Host "WoodCheck 1.0.0 编译完成。" -ForegroundColor Green
