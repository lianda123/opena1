...
$solution = Join-Path $projectRoot "WoodExport.sln"
$project = Join-Path $projectRoot "src\WoodExport\WoodExport.csproj"
$dist = Join-Path $projectRoot "dist"
$net48Out = Join-Path $projectRoot "src\WoodExport\bin\$Configuration\net48"
$net8Out = Join-Path $projectRoot "src\WoodExport\bin\$Configuration\net8.0"
...
$rhino7Plugin = Join-Path $net48Out "WoodExport.rhp"
$rhino8Plugin = Join-Path $net8Out "WoodExport.rhp"
...
Copy-Item -LiteralPath $rhino7Plugin -Destination (Join-Path $dist "net48\WoodExport.rhp")
Copy-Item -LiteralPath $rhino8Plugin -Destination (Join-Path $dist "net8.0\WoodExport.rhp")

foreach ($extension in @("pdb", "xml")) {
  $file7 = Join-Path $net48Out "WoodExport.$extension"
  $file8 = Join-Path $net8Out "WoodExport.$extension"
...
$rhino7Zip = Join-Path $dist "WoodExport-1.0.0-rhino7.zip"
Compress-Archive -Path (@((Join-Path $dist "net48")) + $common) -DestinationPath $rhino7Zip -Force

$rhino8Zip = Join-Path $dist "WoodExport-1.0.0-rhino8.zip"
Compress-Archive -Path (@((Join-Path $dist "net8.0")) + $common) -DestinationPath $rhino8Zip -Force

$combinedZip = Join-Path $dist "WoodExport-1.0.0-rhino7-rhino8.zip"
...
Write-Host "WoodExport 1.0.0 编译完成。" -ForegroundColor Green
